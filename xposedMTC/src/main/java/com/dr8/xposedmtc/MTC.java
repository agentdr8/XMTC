...
public class MTC implements IXposedHookLoadPackage, IXposedHookZygoteInit {
	...
	public void handleLoadPackage(LoadPackageParam lpparam) throws Throwable {
		...
		if (lpparam.packageName.equals(targetmtc)) {
			...
			if (prefs.getBoolean("allbtobd", false)) {
				if (getAndroidRelease().matches("4.2.*"))
					findAndHookMethod(SETTINGS_CLASS, lpparam.classLoader, "connecttodevice", String.class, String.class, new XC_MethodReplacement() {
						...
						protected Object replaceHookedMethod(MethodHookParam mparam) throws Throwable {
							int i = 1;
							String paramString1 = (String) mparam.args[0];
							Intent localIntent = new Intent("com.microntek.settingsaccess");
							localIntent.putExtra("accesstype", 5);
							String str1 = "";
							int j = 12 - paramString1.length();
							for (int k = 0; k < j; k++)
								str1 = str1 + "0";
							String str2 = str1 + paramString1;
							if (i != 0) {
								for (String str3 = "AT-PD" + str2 + "\r\n"; ; str3 = "AT-CC" + str2 + "\r\n")
								{
									Log.i("Settings", "write " + str3);
									localIntent.putExtra("accesscommand", str3);
									mCtx.sendBroadcast(localIntent);
								}
							}
							return null;
						}
			...
			if (prefs.getBoolean("allbtobd", false)) {
				if (getAndroidRelease().matches("4.2.*")) {
					findAndHookMethod(BTUI_CLASS, lpparam.classLoader, "isOBDDevice", new XC_MethodHook() {
						@Override
						protected void afterHookedMethod(final MethodHookParam mparam) throws Throwable { 
							mparam.setResult(true);
						}
